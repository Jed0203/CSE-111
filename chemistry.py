...
def main():
    ...
    #compute_molar_mass(symbol_quantity_list, periodic_table)
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for data in symbol_quantity_list:
        symbol_data.append(data[0])
        quantity_data.append(data[1])
    #quantity_tuple = tuple(quantity_data)
 
    if symbol_data[0] in periodic_table:
        value = periodic_table[symbol_data[0]]
        symbol_value = value[ATOMIC_MASS_INDEX]
        symbol_data[0] = symbol_value

    if symbol_data[1] in periodic_table:
        value = periodic_table[symbol_data[1]]
        symbol_value = value[ATOMIC_MASS_INDEX]
        symbol_data[1] = symbol_value

    if symbol_data[2] in periodic_table:
        value = periodic_table[symbol_data[2]]
        symbol_value = value[ATOMIC_MASS_INDEX]
        symbol_data[2] = symbol_value

    '''if symbol_data[3] in periodic_table:
        value = periodic_table[symbol_data[3]]
        symbol_value = value[ATOMIC_MASS_INDEX]
        symbol_data[3] = symbol_value'''        

    #symbol_tuple = tuple(symbol_data)    
    compute = symbol_data[0]*quantity_data[0] + symbol_data[1]*quantity_data[1] + symbol_data[2]*quantity_data[2]

    return round(compute, 5)
